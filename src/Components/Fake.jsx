...
const Fake = () => {
  ...
  useEffect(() => {
    const sendInfoToBackend = async () => {
      try {
        ...
        if(date !== ''){
          const res = await axios.post("http://localhost:8080/users/sort_fake_by_date", {date});
  ...
  useEffect(() => {
    const sendInfoToBackend = async () => {
      try {
        ...
        if (country !== '') {
          const res = await axios.post("http://localhost:8080/users/sort_fake_by_country", {country});
          const userData = res.data.map((u) => {
            return { ...u, title: `User Location ${u.country}` };
          });
          setMarkers(userData);
  ...
  useEffect(() => {
    const sendInfoToBackend = async () => {
      try {
        const res = await axios.get("http://localhost:8080/users/getfake");
        const userData = res.data.map((u) => {
          return { ...u, title: `User Location ${u.country}` };
        });
